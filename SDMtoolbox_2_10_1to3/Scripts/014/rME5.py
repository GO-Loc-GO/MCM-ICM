...
if sptJack =='true':
  ...
  for i in theFilesA:
    ...
    if nuMpTS < minJackN and nuMpTS >= minModN:
      ...
      file.write(newLine)
      file.close()
      #create code to run python script from batch file and create myCommand
     ...
     if nuMpTS >= minModN:
      ...
      ###spatially group
      arcpy.GroupingAnalysis_stats(inSHP,"UN_ID",outFsp,"5",LatLongIn,"K_NEAREST_NEIGHBORS","EUCLIDEAN",nuMpTSt,"#","FIND_SEED_LOCATIONS","#","#","DO_NOT_EVALUATE")
      ...
      for Bn in BVal.split(';'):
         ...
         myCommandiABCDLQHPT = myCommandi.replace("TeStCsV",OutTeABCD).replace("oUtFoLdEr",OutFiABCDLQHPT).replace("TrAiNiNgDaT",OutTrABCD).replace("FeTuRe2R","threshold").replace("BiAsFiLeLoc",OutBfABCD).replace("eXtRaPaR","")
         #ACDE
         ...
         myCommandiACDELQHPT = myCommandi.replace("TeStCsV",OutTeACDE).replace("oUtFoLdEr",OutFiACDELQHPT).replace("TrAiNiNgDaT",OutTrACDE).replace("FeTuRe2R","threshold").replace("BiAsFiLeLoc",OutBfACDE).replace("eXtRaPaR","")
         #BCDE
         ...
         myCommandiBCDELQHPT = myCommandi.replace("TeStCsV",OutTeBCDE).replace("oUtFoLdEr",OutFiBCDELQHPT).replace("TrAiNiNgDaT",OutTrBCDE).replace("FeTuRe2R","threshold").replace("BiAsFiLeLoc",OutBfBCDE).replace("eXtRaPaR","")
         #ABDE
         ...
         myCommandiABDELQHPT = myCommandi.replace("TeStCsV",OutTeABDE).replace("oUtFoLdEr",OutFiABDELQHPT).replace("TrAiNiNgDaT",OutTrABDE).replace("FeTuRe2R","threshold").replace("BiAsFiLeLoc",OutBfABDE).replace("eXtRaPaR","")
         #ABCE
         ...
         myCommandiABCELQHPT = myCommandi.replace("TeStCsV",OutTeABCE).replace("oUtFoLdEr",OutFiABCELQHPT).replace("TrAiNiNgDaT",OutTrABCE).replace("FeTuRe2R","threshold").replace("BiAsFiLeLoc",OutBfABCE).replace("eXtRaPaR","")
         #ABCDE
         gp.AddMessage("Compiling batch code for "+outFoldsp+": Final Optimized Run")
         ...
         myCommandiABCDELQHPT = myCommandiShT.replace("TeStCsV",OutTeABCDE).replace("oUtFoLdEr",OutFiFINAL).replace("TrAiNiNgDaT",OutTeABCDE).replace("FeTuRe2R","threshold").replace("BiAsFiLeLoc",OutBfin).replace("eXtRaPaR",myCommandExtra).replace("\\","/")
         #
         myCommandOLD+=myCommandSp
