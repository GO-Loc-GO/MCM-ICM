  ...
  for i in theFilesA:
    ...
    if nuMpTS < minJackN and nuMpTS >= minModN:
     ...
     if nuMpTS >= minModN:
      ...
      if nuMpTSt == int((nuMpTS+2)/3): #equals even break  
         nuMpTSt1=nuMpTSt
         nuMpTSt2=nuMpTSt   
      if nuMpTSt != int((nuMpTS+2)/3) and nuMpTSt == int((nuMpTS+1)/3): #equals +1
         nuMpTSt1=nuMpTSt+1
         nuMpTSt2=nuMpTSt
      if nuMpTSt != int((nuMpTS+2)/3) and nuMpTSt != int((nuMpTS+1)/3):#equals +2
         nuMpTSt1=nuMpTSt+1
         nuMpTSt2=nuMpTSt+1   
      ###spatially group
      ...
      arcpy.CopyFeatures_management("G_Sp_Thiessen",outFspPoly_3)
      arcpy.SelectLayerByAttribute_management("G_Sp_Thiessen", "CLEAR_SELECTION")
      arcpy.CalculateField_management(outFspPoly_3,"SS_GROUP", "3","#")
      outFspPoly_Merge=str(outFspPoly_1)+";"+str(outFspPoly_2)+";"+str(outFspPoly_3)
      arcpy.Merge_management(outFspPoly_Merge,outFspPoly_F,"")  
      mxd = arcpy.mapping.MapDocument("CURRENT")
      for df in arcpy.mapping.ListDataFrames(mxd):
        for lyr in arcpy.mapping.ListLayers(mxd,"G_Sp_*", df):
            if lyr.name.lower() != "":
      ...
      #****************************************************
      #create code to run python script4 from batch file
      RunPyAHd4=("start "+PyLocT+" "+outFoldGIS+"\\FinalRank.py").replace("/","\\")
      RunPyAHd4+=" \n"
      ##create summary stats
      csvfile = outFoldspF+"/"+outFoldsp+"_SUMSTATS_ALL.csv"
      newLine = ""
      ...
      for Bn in BVal.split(';'):
         ...
         myCommandiABLQHPT = myCommandi.replace("TeStCsV",OutTeAB).replace("oUtFoLdEr",OutFiABLQHPT).replace("TrAiNiNgDaT",OutTrAB).replace("eTuRe2R","nothreshold").replace("BiAsFiLeLoc",OutBfAB).replace("eXtRaPaR","")
         #AC
         ...
         myCommandiACLQHPT = myCommandi.replace("TeStCsV",OutTeAC).replace("oUtFoLdEr",OutFiACLQHPT).replace("TrAiNiNgDaT",OutTrAC).replace("eTuRe2R","nothreshold").replace("BiAsFiLeLoc",OutBfAC).replace("eXtRaPaR","")
         #BC
         ...
         myCommandiBCLQHPT = myCommandi.replace("TeStCsV",OutTeBC).replace("oUtFoLdEr",OutFiBCLQHPT).replace("TrAiNiNgDaT",OutTrBC).replace("eTuRe2R","nothreshold").replace("BiAsFiLeLoc",OutBfBC).replace("eXtRaPaR","")
         #ABC
         gp.AddMessage("Compiling batch code for "+outFoldsp+": Final Optimized Run")
         ...
         myCommandiABCLQHPT = myCommandiShT.replace("TeStCsV",OutTeABC).replace("oUtFoLdEr",OutFiFINAL).replace("TrAiNiNgDaT",OutTeABC).replace("eTuRe2R","nothreshold").replace("BiAsFiLeLoc",OutBfin).replace("eXtRaPaR",myCommandExtra).replace("\\","/")
         #
         myCommandOLD+=myCommandSp
