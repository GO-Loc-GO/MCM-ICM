...
#
import os, sys, string, arcpy  #common examples
...
gp, gp_version = gp_create(10.1)
gp.AddMessage("version " + str(gp_version))
gp, msg, passed = gp_toolboxes(["Data Management Tools.tbx"], gp, gp_version)


outFolder2 = (outFolder + "/TEMP_ilUlii").replace("\\","/")
OutShape1 = outFolder + "/points_unique_haps.shp"
inFC = OutShape1
...
for aVal in valueList:
  ...
  try:
    ...
    aSuffix = aSuffix.replace("-","")
    outName = aSuffix + ".shp"
    outShapeFile = outFolder2 + "/" + outName
...
#
arcpy.Delete_management(outFolder + "/points_unique_haps.shp")
gp.AddMessage("\n  Processing complete" + "\n")
del row
del rows
del gp